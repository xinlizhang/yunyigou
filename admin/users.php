...
elseif ($_REQUEST['act'] == 'toggle_is_agency')
{
    check_authz_json('users_manage');

    $user_id       = intval($_POST['id']);
    $is_agency     = intval($_POST['val']);

    if ($exc->edit("is_agency = '$is_agency'" , $user_id))
    {
        make_json_result($is_agency);
    }
}
/*------------------------------------------------------ */
//-- 删除会员帐号
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, is_agency, user_money, frozen_money, rank_points, pay_points, reg_time ".
