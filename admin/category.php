...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 分类的图片管理 */
    /* 检查图片：如果有错误，检查尺寸是否超过最大值；否则，检查文件类型；maxsize最大上传文件大小 */
    ...
    if (isset($_FILES['category_icon']))
    {
        ...
        {
            if (!$image->check_img_type($_FILES['category_icon']['type']))
            {
                sys_msg($_LANG['invalid_category_icon'], 1, array(), false);
            }
        }
        elseif ($_FILES['category_icon']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['category_icon_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['category_icon']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['category_icon_too_big'], $htm_maxsize), 1, array(), false);
        }

        // 上传操作
        if ( ($_FILES['category_icon']['tmp_name'] != '' && $_FILES['category_icon']['tmp_name'] != 'none') )
        {
            if ( empty($_POST['category_icon_url']) || $_POST['category_icon_url'] == $_LANG['lab_icon_url'] || $_POST['category_icon_url'] == 'http://' )
            {
                $cat['category_icon'] = $image->upload_image_2_images($_FILES['category_icon'], 'Category/category_icon');
            }
            else
            {
                if(preg_match('/(.jpg|.png|.gif|.jpeg)$/',$_POST['category_icon_url'])
                    && copy(trim($_POST['category_icon_url']), ROOT_PATH . 'temp/' . basename($_POST['category_icon_url'])))
                {
                    $cat['category_icon'] = 'temp/' . basename($_POST['category_icon_url']);
                }
            }

            if ($cat['category_icon'] === false)
            {
    ...
    if (isset($_FILES['category_img']))
    {
        ...
        {
            if (!$image->check_img_type($_FILES['category_img']['type']))
            {
                sys_msg($_LANG['invalid_category_img'], 1, array(), false);
            }
        }
        elseif ($_FILES['category_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['category_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['category_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['category_img_too_big'], $htm_maxsize), 1, array(), false);
        }

        // 上传操作
        if ( ($_FILES['category_img']['tmp_name'] != '' && $_FILES['category_img']['tmp_name'] != 'none') )
        {
            if ( empty($_POST['category_img_url']) || $_POST['category_img_url'] == $_LANG['lab_picture_url'] || $_POST['category_img_url'] == 'http://' )
            {
                $cat['category_img'] = $image->upload_image_2_images($_FILES['category_img'], 'Category/category_img');
            }
            else
            {
                if(preg_match('/(.jpg|.png|.gif|.jpeg)$/',$_POST['category_img_url'])
                    && copy(trim($_POST['category_img_url']), ROOT_PATH . 'temp/' . basename($_POST['category_img_url'])))
                {
                    $cat['category_img'] = 'temp/' . basename($_POST['category_img_url']);
                }
            }

            if ($cat['category_img'] === false)
            {
                sys_msg($image->error_msg(), 1, array(), false);
            }
        }
    ...
    // ------ Thumb
    if (isset($_FILES['category_thumb']))
    {
        if ($_FILES['category_thumb']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['category_thumb']['type']))
            {
                sys_msg($_LANG['invalid_category_thumb'], 1, array(), false);
            }
        }
        elseif ($_FILES['category_thumb']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['category_thumb_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['category_thumb']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['category_thumb_too_big'], $htm_maxsize), 1, array(), false);
        }

        // 上传操作
        if ( ($_FILES['category_thumb']['tmp_name'] != '' && $_FILES['category_thumb']['tmp_name'] != 'none') )
        {
            if ( empty($_POST['category_thumb_url']) || $_POST['category_thumb_url'] == $_LANG['lab_thumb_url'] || $_POST['category_thumb_url'] == 'http://' )
            {
                $cat['category_thumb'] = $image->upload_image_2_images($_FILES['category_thumb'], 'Category/category_thumb');
            }
            else
            {
                if(preg_match('/(.jpg|.png|.gif|.jpeg)$/',$_POST['category_thumb_url'])
                    && copy(trim($_POST['category_thumb_url']), ROOT_PATH . 'temp/' . basename($_POST['category_thumb_url'])))
                {
                    $cat['category_thumb'] = 'temp/' . basename($_POST['category_thumb_url']);
                }
            }

            if ($cat['category_thumb'] === false)
            {
                sys_msg($image->error_msg(), 1, array(), false);
            }
        }
...
if ($_REQUEST['act'] == 'update')
{
    ...
    /* 修改分类与添加分类的不同之处：修改需要记录下原来图片的值 */
    $cat['category_icon']   = !empty($_POST['category_icon_old'])       ? trim($_POST['category_icon_old'])         : '';
    $cat['category_img']    = !empty($_POST['category_img_old'])        ? trim($_POST['category_img_old'])          : '';
    $cat['category_thumb']  = !empty($_POST['category_thumb_old'])      ? trim($_POST['category_thumb_old'])        : '';
    ...
    $dat = $db->getRow("SELECT * FROM ". $ecs->table('category') . " WHERE cat_id = '$cat_id'");
    ...
    if ($db->autoExecute($ecs->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        ...
        if($cat['category_icon'] != $dat['category_icon'])
        {
            // 分类Icon发生了变化
            if ($dat['category_icon'] != '' && is_file('../' . $dat['category_icon']))
            {
                @unlink('../' . $dat['category_icon']);
            }
        }
        if($cat['category_img'] != $dat['category_img'])
        {
            // 分类Img发生了变化
            if ($dat['category_img'] != '' && is_file('../' . $dat['category_img']))
            {
                @unlink('../' . $dat['category_img']);
            }
        }
        if($cat['category_thumb'] != $dat['category_thumb'])
        {
            // 分类Icon发生了变化
            if ($dat['category_thumb'] != '' && is_file('../' . $dat['category_thumb']))
            {
                @unlink('../' . $dat['category_thumb']);
            }
        }
...
//-- 首页栏目分类带商品
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'category_goods_list')
{
    /* 获取分类列表 */
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE parent_id = 0 ORDER BY cat_id ASC ';
	$cat_list = $GLOBALS['db']->getAll($sql);

    /* 模板赋值 */
    $smarty->assign('ur_here',      $_LANG['category_goods_list']);
    $smarty->assign('action_link',  array('href' => 'category.php?act=add', 'text' => $_LANG['category_goods_list']));
    $smarty->assign('full_page',    1);
    ...
    /* 列表页面 */
    assign_query_info();
    $smarty->display('category_goods_list.htm');
}
...
if ($_REQUEST['act'] == 'category_goods_is_show')
{
    check_authz_json('cat_manage');
    ...
    if (cat_update($id, array('show_in_index' => $val)) != false)
    {
        if($val == 1)
        {
            
            $db->query("UPDATE " . $ecs->table('category') . " SET show_in_index = 1 WHERE cat_id = " . $id );
        }
        else
        {
            //去除
            $db->query("UPDATE " . $ecs->table('category') . " SET show_in_index = 0 WHERE cat_id = " . $id );
        }
        clear_cache_files();
        make_json_result($val);
    }
...
if ($_REQUEST['act'] == 'category_goods_sort_order')
{
    check_authz_json('cat_manage');
    ...
    if (cat_update($id, array('sort_order_index' => $val)))
    {
        clear_cache_files(); // 清除缓存
        make_json_result($val);
    }
