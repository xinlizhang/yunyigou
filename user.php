...
elseif ($action == 'order_list')
{
	...
	switch($filter['composite_status'])
	{
		...
		case PS_PAYED :
			...
			{
				$where .= " AND pay_status = '$filter[composite_status]' ";
			}
			break;
		case SS_SHIPPED :
			if ($filter['composite_status'] != -1)
			{
				$where .= " AND shipping_status  = '$filter[composite_status]'-2 ";
			}
			break;
		default:
			if ($filter['composite_status'] != -1)
			{
				$where .= " AND order_status = '$filter[composite_status]' ";
			}
	}
    ...
    $smarty->assign('ps_unpayed',     PS_UNPAYED);
    $smarty->assign('ps_payed',    PS_PAYED);
