...
class cls_session
{
    ...
    var $max_life_time  = 3600;       // 登录用户SESSION有效期 60*60*1
    ...
    function update_session()
    {
        ...
        //if (isset($data{255}))
        //{
            //$this->db->autoReplace($this->session_data_table, array('sesskey' => $this->session_id, 'expiry' => $this->_expiry_time, 'data' => $data), array('expiry' => $this->_expiry_time,'data' => $data));

            //$data = '';
        //}

        return $this->db->query('UPDATE ' . $this->session_table . " SET expiry = '" . $this->_expiry_time . "', ip = '" . $this->_ip . "', userid = '" . $userid . "', adminid = '" . $adminid . "', user_name='" . $user_name . "', user_rank='" . $user_rank . "', discount='" . $discount . "', email='" . $email . "', data = '$data' WHERE sesskey = '" . $this->session_id . "' LIMIT 1");
    }
    ...
    function close_session()
    {
        ...
        if (mt_rand(0, 2) == 2)
        {
            $this->db->query('DELETE FROM ' . $this->session_data_table . ' WHERE expiry < ' . $this->_time);
        }

        if ((time() % 2) == 0)
        {
            return $this->db->query('DELETE FROM ' . $this->session_table . ' WHERE expiry < ' . $this->_time);
        }
