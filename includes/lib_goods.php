    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['category_icon'] = $row['category_icon'];
                $cat_arr[$row['cat_id']]['category_img'] = $row['category_img'];
                $cat_arr[$row['cat_id']]['category_thumb'] = $row['category_thumb'];
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show, category_icon, category_img, category_thumb ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $three_arr[$row['cat_id']]['category_icon'] = $row['category_icon'];
                $three_arr[$row['cat_id']]['category_img'] = $row['category_img'];
                $three_arr[$row['cat_id']]['category_thumb'] = $row['category_thumb'];
 ...
 * @return  array
 */
function get_recommend_goods($type = '', $sort_by = 'default', $cats = '')
{
...
function get_top4_7($cats = '')
{
    ...
    $sql .= ' AND og.order_id = o.order_id AND og.goods_id = g.goods_id ' .
           ...
           "AND (o.pay_status = '" . PS_PAYED . "' OR o.pay_status = '" . PS_PAYING . "') " .
           "AND (o.shipping_status = '" . SS_SHIPPED . "' OR o.shipping_status = '" . SS_RECEIVED . "') " .
           'GROUP BY g.goods_id ORDER BY goods_number DESC, g.goods_id DESC LIMIT 3,4';
 ...
 *
 * @access  public
 * @param   integer     $parent_id     分类编号
 * @return  array
 */
function get_category_in_index($num = 0)
{
    $parent_id = 0;

    /* 获取当前分类及其子分类 */
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('category') . ' WHERE parent_id = '.$parent_id.' AND show_in_index = 1 ORDER BY sort_order_index ASC ';

    $res = $GLOBALS['db']->getAll($sql);

    foreach ($res AS $row)
    {
        if ($row['show_in_index'])
        {
            $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
            $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
            $cat_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
            ...
            if (isset($row['cat_id']) != NULL)
            {
                $cat_arr[$row['cat_id']]['cat_goods'] = get_category_goods_list($row['cat_id'], $num);
                $cat_arr[$row['cat_id']]['cat_child_tree'] = get_child_tree($row['cat_id']);
                $cat_arr[$row['cat_id']]['get_sales'] = get_top10($row['cat_id']);
            }
        }
    }
 ...
 * @return  array
 */
function get_category_goods_list($cat_id = 0, $num)
{
	$sql = 'Select g.goods_id, g.cat_id,c.parent_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.is_shipping, ' .  
           ...
           "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".  
           "Where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ";  
    $sql .= " AND (c.parent_id =" . $cat_id. " OR g.cat_id = " . $cat_id ." OR g.cat_id ". db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) .") ORDER BY g.goods_id DESC LIMIT " . $num; 
	$res = $GLOBALS['db']->getAll($sql);
    foreach ($res AS $idx => $row)  
    {  
        $goods[$idx]['id'] = $row['goods_id'];  
