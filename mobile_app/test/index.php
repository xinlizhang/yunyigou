...
<body>
	...
	<div class="parms_div">
		...
		<select id="serverUrl" autocomplete="off">
			<option value="/yunyigou/mobile_app/?url=" selected>当前域名</option>
...
<div class="parms_div">
					...
					<?php foreach ($value['request'] as $key => $value): ?>
						<?php
							if (is_string($value) && strpos($value, '{') !== false && strpos($value, '}') !== false)
							{
                                ...
                                // trm(string,charlist) 删除string两端的charlist字符串
                                // strtoupper(string) 把字符串钻尾
    ...
    <div class="parms_div">
        <span>请求URL：</span><br/>
        <div id="request_URL" style="result">
        </div>
    </div>
    <div class="parms_div">
        <span>请求JSON：</span><br/>
        <div id="request_Json" style="result">
        </div>
    </div>
	<div class="parms_div">
		<span>返回结果：</span><br/>
		...
		$("#send").click(function(){
			...
			$.ajax({
				...
				'beforeSend' : function(){
                    ...
                    $("#request_Json").html(JsonUti.convertToString({'json': JSON.stringify(parm)}));
					$("#request").html('加载数据中...');
				},
