...
function    GZ_user_info($user_id)
{
    ...
    $cart_num = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('cart')." WHERE user_id='$user_id' ORDER BY rec_id ASC");
...
function GZ_order_query_sql($type = 'finished', $alias = '')
{
    ...
    if ($type == 'finished')
    {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
               ...
               " AND ( {$alias}pay_status " . db_create_in(array(PS_PAYED, PS_PAYING)) . " OR ({$alias}pay_status " . db_create_in(PS_UNPAYED) . "AND pay_id " . db_create_in(PAY_COD) . ")) ";
    }
    /* 待发货订单 */
	...
	switch ($type) {
		...
		case 'GOODS':
			...
			$outData = array(
				...
				"promote_start_date" => $readData['promote_start_date'],
				"promote_end_date"  => $readData['promote_end_date'],
                "is_shipping" => $readData['is_shipping'],
				"is_promote" => $readData['is_promote'],
				"img" => array(
					'thumb'=>API_DATA('PHOTO', $readData['goods_img']),
					'url' => API_DATA('PHOTO', $readData['original_img']),
					'small'=>API_DATA('PHOTO', $readData['goods_thumb'])
