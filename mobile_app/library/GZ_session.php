...
class GZ_session
{
    ...
    var $max_life_time  = 2592000;       // 登录用户SESSION有效期 60*60*24*30
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        if ($this->session_id)
        {
            // 取到了cookie或形参的值 进行合法性验证
            // gen_session_key包含了IP地址验证 即验证当前IP发来的session_id是否为当前IP生成
        ...
        $this->_time = time();
        $this->_expiry_time = $this->_time + $this->max_life_time;

        if ($this->session_id)
        {
            // 若当前session_id有效 则去数据库检索相关字段
            $this->load_session();
        }
        else
        {
    ...
    // session刚创建时 插入数据库 此时均为无用户状态 后续close时会根据用户情况更新
    function insert_session()
    {
        $sql = 'INSERT INTO ' . $this->session_table
                . " (sesskey, expiry, ip, data) VALUES ('" . $this->session_id . "', '". $this->_expiry_time ."', '". $this->_ip ."', 'a:0:{}')";
    ...
    function update_session()
    {
        ...
        // 10s内不更新
        if ($this->session_md5 == md5($data) && $this->_time < ($this->session_expiry-$this->max_life_time) + 10)
        {
            return true;
        }

        $data = addslashes($data) . "Log1" . $this->_test;

        $sql = 'UPDATE ' . $this->session_table
            . " SET expiry = '" . $this->_expiry_time . "', ip = '" . $this->_ip . "', userid = '" . $userid . "', adminid = '" . $adminid . "', user_name='". $user_name
            . "', user_rank='" . $user_rank . "', discount='" . $discount . "', email='" . $email . "', data = '$data' WHERE sesskey = '" . $this->session_id . "' LIMIT 1";
    ...
    function close_session()
    {
        ...
        if (mt_rand(0, 2) == 2)
        {
            ...
            $sql = "SELECT * FROM " . $this->session_table
                . ' WHERE expiry < ' . $this->_time
                . ' AND userid = 0';
            $session_list =  $GLOBALS['db']->getAll($sql);
            foreach ($session_list as $key => $value)
            {
                $this->db->query('DELETE FROM ' . $this->session_table . ' WHERE sesskey = ' . "'$value[sesskey]'");
                $this->db->query('DELETE FROM ' . $this->session_data_table . ' WHERE sesskey = ' . "'$value[sesskey]'");
            }
