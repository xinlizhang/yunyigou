...
//购物车数量加减
function goods_cut($rec_id,$goods_id){
	...
	if(Num>1)Num=Num-1;
	num_val.value=Num;
	submit_update_cart($rec_id,$goods_id).click();
}
function goods_add($rec_id,$goods_id){
	var num_val=document.getElementById('goods_number_'+$rec_id);
	var new_num=num_val.value;
	if(isNaN(new_num)){alert('请输入数字');return false}
	var Num = parseInt(new_num);
...
function submit_update_cart(rec_id,goods_id){
	var goods_number = document.getElementById("goods_number_"+rec_id).value;
	Ajax.call('flow.php?step=ajax_update_cart', 'goods_number=' + goods_number + '&rec_id='+ rec_id +'&goods_id='+goods_id, submit_update_cartResponse_cart, 'GET', 'JSON');
}
